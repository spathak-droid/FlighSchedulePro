...
export const MOCK_STUDENTS: FspStudent[] = [
  { id: 'stu-001', firstName: 'Alex', lastName: 'Johnson', fullName: 'Alex Johnson', email: 'alex.j@email.com' },
  { id: 'stu-002', firstName: 'Emily', lastName: 'Davis', fullName: 'Emily Davis', email: 'emily.d@email.com' },
  { id: 'stu-003', firstName: 'Ryan', lastName: 'Martinez', fullName: 'Ryan Martinez', email: 'ryan.m@email.com' },
  { id: 'stu-004', firstName: 'Sophie', lastName: 'Brown', fullName: 'Sophie Brown', email: 'sophie.b@email.com' },
  { id: 'stu-005', firstName: 'Tyler', lastName: 'Lee', fullName: 'Tyler Lee', email: 'tyler.l@email.com' },
  { id: 'stu-006', firstName: 'Mia', lastName: 'Garcia', fullName: 'Mia Garcia', email: 'mia.g@email.com' },
];
...
export function generateReservationListItems(): FspReservationListItem[] {
  ...
  return events.map((ev, i) => ({
    ...
    pilotId: MOCK_STUDENTS.find((s) => `${s.firstName} ${s.lastName}` === ev.CustomerName)?.id ?? 'stu-unknown',
...
export const MOCK_AVAILABILITY: Record<string, FspAvailability> = {
  ...
  'inst-001': {
    ...
    availabilityOverrides: [
      ...
      { date: isoDate(futureDate(5, 0)), startTime: '07:00:00', endTime: '17:00:00', isUnavailable: true },
    ],
  },
  'inst-002': {
    userGuidId: 'inst-002',
  ...
  'inst-003': {
    ...
    availabilityOverrides: [
      ...
      { date: isoDate(futureDate(3, 0)), startTime: '07:00:00', endTime: '12:00:00', isUnavailable: true },
    ],
  },
...
export const MOCK_AIRCRAFT_TIMES: Record<string, {
  aircraftId: string; totalTime: number; hobbs: number; tach: number; lastFlightDate: string;
}> = {
  'ac-001': { aircraftId: 'ac-001', totalTime: 4520.3, hobbs: 4520.3, tach: 3890.1, lastFlightDate: fspLocal(futureDate(0, 16, 0)) },
  'ac-002': { aircraftId: 'ac-002', totalTime: 8210.7, hobbs: 8210.7, tach: 7105.2, lastFlightDate: fspLocal(futureDate(0, 14, 30)) },
  'ac-003': { aircraftId: 'ac-003', totalTime: 3150.5, hobbs: 3150.5, tach: 2890.8, lastFlightDate: fspLocal(futureDate(-1, 17, 0)) },
  'ac-004': { aircraftId: 'ac-004', totalTime: 1200.0, hobbs: 1200.0, tach: 1200.0, lastFlightDate: fspLocal(futureDate(0, 18, 0)) },
};

export const MOCK_AIRCRAFT_SQUAWKS: Record<string, Array<{
  id: string; aircraftId: string; description: string; reportedBy: string;
  reportedDate: string; status: string; severity: string;
}>> = {
  'ac-001': [
    {
      id: 'sqk-001', aircraftId: 'ac-001',
      description: 'Slight vibration in right rudder pedal at taxi speed',
      reportedBy: 'James Wilson', reportedDate: fspLocal(futureDate(-3, 10, 0)),
      status: 'deferred', severity: 'non-grounding',
    },
  ],
  'ac-002': [],
  'ac-003': [
    {
      id: 'sqk-002', aircraftId: 'ac-003',
      description: 'GPS database expired — needs NavData update',
      reportedBy: 'Lisa Park', reportedDate: fspLocal(futureDate(-2, 9, 0)),
      status: 'open', severity: 'non-grounding',
    },
  ],
  'ac-004': [],
};

export const MOCK_MAINTENANCE_REMINDERS: Record<string, Array<{
  id: string; aircraftId: string; type: string; description: string;
  dueDate: string | null; dueHobbs: number | null; currentHobbs: number;
  remainingHours: number | null; status: string;
}>> = {
  'ac-001': [
    {
      id: 'maint-001', aircraftId: 'ac-001', type: '100-hour',
      description: '100-hour inspection', dueDate: null, dueHobbs: 4600,
      currentHobbs: 4520.3, remainingHours: 79.7, status: 'upcoming',
    },
    {
      id: 'maint-002', aircraftId: 'ac-001', type: 'annual',
      description: 'Annual inspection', dueDate: fspLocal(futureDate(45, 8, 0)),
      dueHobbs: null, currentHobbs: 4520.3, remainingHours: null, status: 'upcoming',
    },
  ],
  'ac-002': [
    {
      id: 'maint-003', aircraftId: 'ac-002', type: '100-hour',
      description: '100-hour inspection', dueDate: null, dueHobbs: 8250,
      currentHobbs: 8210.7, remainingHours: 39.3, status: 'due_soon',
    },
  ],
  'ac-003': [
    {
      id: 'maint-004', aircraftId: 'ac-003', type: 'oil_change',
      description: 'Oil and filter change', dueDate: null, dueHobbs: 3200,
      currentHobbs: 3150.5, remainingHours: 49.5, status: 'upcoming',
    },
  ],
  'ac-004': [],
};
...
export const MOCK_SCHEDULE_FILTERS = {
  ...
  aircraft: MOCK_AIRCRAFT.map((a) => ({ id: a.id, name: `${a.registration} - ${a.makeModel}`, isActive: a.isActive })),
  activityTypes: MOCK_ACTIVITY_TYPES.map((at) => ({ id: at.id, name: at.name, isActive: at.isActive })),
...
export function generateAutoScheduleResults(
  requestBody?: { events?: Array<{ eventId: string; customer1Guid?: string }> },
) {
  const schedulable = generateSchedulableEvents();
  const sourceEvents = requestBody?.events;
  const eventsToPlace = sourceEvents && sourceEvents.length > 0
    ? sourceEvents
    : schedulable.map((e) => ({ eventId: e.eventId, customer1Guid: e.studentId }));

  return {
    events: eventsToPlace.map((ev, i) => {
      const se = schedulable.find((s) => s.eventId === ev.eventId) ?? schedulable[i % schedulable.length]!;
      return {
        eventId: ev.eventId,
  ...
  return [
    {
      ...
      instructorId: 'inst-001', instructorName: 'James Wilson',
      aircraftId: 'ac-001', aircraftName: 'N172SP', score: 95,
    },
    {
      startTime: fspLocal(futureDate(1, 14, 0)),
      endTime: fspLocal(futureDate(1, 16, 0)),
      instructorId: 'inst-002', instructorName: 'Lisa Park',
      aircraftId: 'ac-001', aircraftName: 'N172SP', score: 88,
    },
    {
      startTime: fspLocal(futureDate(2, 9, 0)),
      endTime: fspLocal(futureDate(2, 11, 0)),
      instructorId: 'inst-003', instructorName: 'David Kim',
      aircraftId: 'ac-002', aircraftName: 'N152AB', score: 82,
    },
    {
      startTime: fspLocal(futureDate(2, 13, 0)),
      endTime: fspLocal(futureDate(2, 15, 0)),
      instructorId: 'inst-001', instructorName: 'James Wilson',
      aircraftId: 'ac-003', aircraftName: 'N182RG', score: 78,
    },
    {
      startTime: fspLocal(futureDate(3, 10, 0)),
      endTime: fspLocal(futureDate(3, 12, 0)),
      instructorId: 'inst-002', instructorName: 'Lisa Park',
      aircraftId: 'ac-001', aircraftName: 'N172SP', score: 75,
    },
  ];
}
...
export const MOCK_ENROLLMENT_DETAILS: Record<string, {
  id: string; studentId: string; courseId: string; courseName: string;
  status: string; startDate: string; expectedCompletionDate: string;
  instructorId: string; locationId: string;
}> = {
  'enr-001': {
    id: 'enr-001', studentId: 'stu-001', courseId: 'crs-ppl',
    courseName: 'Private Pilot License', status: 'active',
    startDate: fspLocal(futureDate(-90, 8, 0)),
    expectedCompletionDate: fspLocal(futureDate(30, 8, 0)),
    instructorId: 'inst-001', locationId: 'loc-001',
  },
  'enr-002': {
    id: 'enr-002', studentId: 'stu-002', courseId: 'crs-ir',
    courseName: 'Instrument Rating', status: 'active',
    startDate: fspLocal(futureDate(-60, 8, 0)),
    expectedCompletionDate: fspLocal(futureDate(60, 8, 0)),
    instructorId: 'inst-002', locationId: 'loc-001',
  },
  'enr-003': {
    id: 'enr-003', studentId: 'stu-003', courseId: 'crs-ppl',
    courseName: 'Private Pilot License', status: 'active',
...
export function generateStudentProgressReport(enrollmentId?: string) {
  ...
  if (!progress) return { enrollmentId: enrollmentId ?? 'enr-001', completionPercentage: 0, milestones: [] };
  return {
    ...
    milestones: [
      { name: 'First Solo', status: progress.completedLessons >= 15 ? 'completed' : 'pending', lessonOrder: 15 },
      { name: 'Solo Cross Country', status: progress.completedLessons >= 25 ? 'completed' : 'pending', lessonOrder: 25 },
      { name: 'Checkride Prep', status: progress.completedLessons >= 38 ? 'completed' : 'pending', lessonOrder: 38 },
      { name: 'Checkride', status: progress.completedLessons >= 40 ? 'completed' : 'pending', lessonOrder: 40 },
    ],
...
export const MOCK_CHECKRIDE_SCORES = [
  {
    id: 'crs-001', studentId: 'stu-003', studentName: 'Ryan Martinez',
    examType: 'private_pilot', date: fspLocal(futureDate(-10, 9, 0)),
    result: 'pending', score: null,
    examinerId: 'ext-dpe-001', examinerName: 'Robert Chen (DPE)',
  },
];
...
export const MOCK_KNOWLEDGE_TESTS = [
  {
    id: 'kt-001', studentId: 'stu-001', studentName: 'Alex Johnson',
    testType: 'PAR', testName: 'Private Pilot Airplane',
    date: fspLocal(futureDate(-30, 10, 0)), score: 87, passingScore: 70,
    passed: true, expirationDate: fspLocal(futureDate(335, 10, 0)),
  },
  {
    id: 'kt-002', studentId: 'stu-002', studentName: 'Emily Davis',
    testType: 'IRA', testName: 'Instrument Rating Airplane',
    date: fspLocal(futureDate(-15, 14, 0)), score: 92, passingScore: 70,
    passed: true, expirationDate: fspLocal(futureDate(350, 14, 0)),
  },
  {
    id: 'kt-003', studentId: 'stu-003', studentName: 'Ryan Martinez',
    testType: 'PAR', testName: 'Private Pilot Airplane',
    date: fspLocal(futureDate(-45, 10, 0)), score: 91, passingScore: 70,
    passed: true, expirationDate: fspLocal(futureDate(320, 10, 0)),
  },
];
...
export const MOCK_TRAINING_ALERTS = [
  {
    id: 'ta-001', studentId: 'stu-003', studentName: 'Ryan Martinez',
    type: 'checkride_approaching',
    message: 'Ryan Martinez is at lesson 38/40 — checkride prep imminent',
    severity: 'info', createdAt: fspLocal(futureDate(-1, 8, 0)),
  },
  {
    id: 'ta-002', studentId: 'stu-001', studentName: 'Alex Johnson',
    type: 'knowledge_test_expiring',
    message: 'Alex Johnson PAR knowledge test expires in 335 days',
    severity: 'low', createdAt: fspLocal(futureDate(-2, 9, 0)),
  },
  {
    id: 'ta-003', studentId: 'stu-004', studentName: 'Sophie Brown',
    type: 'no_recent_flight',
    message: 'Sophie Brown has not flown in 14 days — currency at risk',
    severity: 'warning', createdAt: fspLocal(futureDate(0, 6, 0)),
  },
];
...
export const MOCK_FLIGHT_ALERTS = [
  {
    id: 'fa-001', reservationId: 'res-001', operatorId: MOCK_OPERATOR_ID,
    aircraftId: 'ac-001', aircraftName: 'N172SP',
    pilotId: 'stu-001', pilotName: 'Alex Johnson',
    instructorId: 'inst-001', instructorName: 'James Wilson',
    type: 'departure', scheduledTime: fspLocal(futureDate(0, 8, 0)),
    status: 'acknowledged', createdAt: fspLocal(futureDate(0, 7, 30)),
  },
  {
    id: 'fa-002', reservationId: 'res-002', operatorId: MOCK_OPERATOR_ID,
    aircraftId: 'ac-001', aircraftName: 'N172SP',
    pilotId: 'stu-001', pilotName: 'Alex Johnson',
    instructorId: 'inst-001', instructorName: 'James Wilson',
    type: 'arrival', scheduledTime: fspLocal(futureDate(0, 10, 0)),
    status: 'pending', createdAt: fspLocal(futureDate(0, 7, 30)),
  },
  {
    id: 'fa-003', reservationId: 'res-003', operatorId: MOCK_OPERATOR_ID,
    aircraftId: 'ac-002', aircraftName: 'N152AB',
    pilotId: 'stu-003', pilotName: 'Ryan Martinez',
    instructorId: 'inst-003', instructorName: 'David Kim',
    type: 'departure', scheduledTime: fspLocal(futureDate(-1, 7, 0)),
    status: 'overdue', createdAt: fspLocal(futureDate(-1, 6, 30)),
  },
  {
    id: 'fa-004', reservationId: 'res-004', operatorId: MOCK_OPERATOR_ID,
    aircraftId: 'ac-003', aircraftName: 'N182RG',
    pilotId: 'stu-002', pilotName: 'Emily Davis',
    instructorId: 'inst-002', instructorName: 'Lisa Park',
    type: 'arrival', scheduledTime: fspLocal(futureDate(0, 16, 0)),
    status: 'pending', createdAt: fspLocal(futureDate(0, 14, 0)),
  },
];
...
export function generateEnrollmentHistory() {
  return [
    { date: fspLocal(futureDate(-90, 9, 0)), action: 'enrolled', details: 'Student enrolled in program', performedBy: 'Sarah Chen' },
    { date: fspLocal(futureDate(-60, 9, 0)), action: 'milestone_reached', details: 'First solo flight completed', performedBy: 'James Wilson' },
    { date: fspLocal(futureDate(-30, 9, 0)), action: 'milestone_reached', details: 'Solo cross-country completed', performedBy: 'James Wilson' },
  ];
}
...
export const MOCK_ALL_OPERATOR_DETAILS: Record<number, FspOperatorDetail> = {
  ...
  1002: {
    id: 1002, name: 'Bay Area Flight Training', isActive: true, isPending: false,
    userId: 'usr-101', firstName: 'John', lastName: 'Rivera', email: 'john@bayareaflight.com',
  },
  1003: {
    id: 1003, name: 'Pacific Coast Aviation', isActive: true, isPending: false,
    userId: 'usr-201', firstName: 'Maria', lastName: 'Santos', email: 'maria@pacificcoast.aero',
  },
};
...
export const MOCK_USERS_BY_OPERATOR: Record<number, FspUser[]> = {
  1001: MOCK_USERS,
  1002: [
    { id: 'usr-101', firstName: 'John', lastName: 'Rivera', fullName: 'John Rivera', email: 'john@bayareaflight.com', role: 'scheduler', isActive: true },
    { id: 'usr-102', firstName: 'Anna', lastName: 'Kowalski', fullName: 'Anna Kowalski', email: 'anna@bayareaflight.com', role: 'admin', isActive: true },
  ],
  1003: [
    { id: 'usr-201', firstName: 'Maria', lastName: 'Santos', fullName: 'Maria Santos', email: 'maria@pacificcoast.aero', role: 'scheduler', isActive: true },
  ],
};
...
export const MOCK_LOGIN_BY_EMAIL: Record<string, { login: FspLoginResponse; operatorId: number }> = {
  ...
  'john@bayareaflight.com': {
    login: {
      ...
      user: { email: 'john@bayareaflight.com', id: 'usr-101', firstName: 'John', lastName: 'Rivera' },
      mfaRequired: false,
    },
    operatorId: 1002,
  },
  'maria@pacificcoast.aero': {
    login: {
      ...
      user: { email: 'maria@pacificcoast.aero', id: 'usr-201', firstName: 'Maria', lastName: 'Santos' },
      mfaRequired: false,
    },
    operatorId: 1003,
  },
...
export const MOCK_LOCATIONS_BY_OPERATOR: Record<number, FspLocation[]> = {
  1001: MOCK_LOCATIONS,
  1002: [
    { id: 'loc-101', name: 'KSQL - San Carlos Airport', code: 'KSQL', timeZone: 'America/Los_Angeles', isActive: true, latitude: 37.5118, longitude: -122.2495 },
  ],
  1003: [
    { id: 'loc-201', name: 'KHWD - Hayward Executive Airport', code: 'KHWD', timeZone: 'America/Los_Angeles', isActive: true, latitude: 37.6590, longitude: -122.1217 },
  ],
};
...
export const MOCK_AIRCRAFT_BY_OPERATOR: Record<number, FspAircraft[]> = {
  1001: MOCK_AIRCRAFT,
  1002: [
    { id: 'ac-101', registration: 'N738JV', make: 'Cessna', model: '172R Skyhawk', makeModel: 'Cessna 172R Skyhawk', isActive: true, isSimulator: false },
    { id: 'ac-102', registration: 'N5412G', make: 'Piper', model: 'PA-28-181 Archer III', makeModel: 'Piper PA-28-181 Archer III', isActive: true, isSimulator: false },
    { id: 'ac-103', registration: 'N611DS', make: 'Diamond', model: 'DA40 Star', makeModel: 'Diamond DA40 Star', isActive: true, isSimulator: false },
  ],
  1003: [
    { id: 'ac-201', registration: 'N921PC', make: 'Cessna', model: '172S Skyhawk SP', makeModel: 'Cessna 172S Skyhawk SP', isActive: true, isSimulator: false },
    { id: 'ac-202', registration: 'N340PC', make: 'Cessna', model: '152', makeModel: 'Cessna 152', isActive: true, isSimulator: false },
  ],
};
...
export const MOCK_INSTRUCTORS_BY_OPERATOR: Record<number, FspInstructor[]> = {
  ...
  1002: [
    { id: 'inst-101', firstName: 'Carlos', lastName: 'Mendez', fullName: 'Carlos Mendez', instructorType: 'CFI', isActive: true },
    { id: 'inst-102', firstName: 'Priya', lastName: 'Sharma', fullName: 'Priya Sharma', instructorType: 'CFII', isActive: true },
  ],
  1003: [
    { id: 'inst-201', firstName: 'Kevin', lastName: 'Tanaka', fullName: 'Kevin Tanaka', instructorType: 'CFI', isActive: true },
  ],
};
...
export const MOCK_STUDENTS_BY_OPERATOR: Record<number, FspStudent[]> = {
  1001: MOCK_STUDENTS,
  1002: [
    { id: 'stu-101', firstName: 'Daniel', lastName: 'Okafor', fullName: 'Daniel Okafor', email: 'daniel.o@email.com' },
    { id: 'stu-102', firstName: 'Rachel', lastName: 'Nguyen', fullName: 'Rachel Nguyen', email: 'rachel.n@email.com' },
    { id: 'stu-103', firstName: 'Marcus', lastName: 'Thompson', fullName: 'Marcus Thompson', email: 'marcus.t@email.com' },
  ],
  1003: [
    { id: 'stu-201', firstName: 'Aisha', lastName: 'Patel', fullName: 'Aisha Patel', email: 'aisha.p@email.com' },
    { id: 'stu-202', firstName: 'Brian', lastName: 'Larsen', fullName: 'Brian Larsen', email: 'brian.l@email.com' },
  ],
};
...
export const MOCK_ENROLLMENTS_BY_OPERATOR: Record<number, Record<string, FspEnrollment[]>> = {
  ...
  1002: {
    'stu-101': [{ id: 'enr-101', studentId: 'stu-101', courseId: 'crs-ppl', courseName: 'Private Pilot License', status: 'active' }],
    'stu-102': [{ id: 'enr-102', studentId: 'stu-102', courseId: 'crs-ppl', courseName: 'Private Pilot License', status: 'active' }],
  },
  1003: {
    'stu-201': [{ id: 'enr-201', studentId: 'stu-201', courseId: 'crs-ppl', courseName: 'Private Pilot License', status: 'active' }],
  },
};
...
export const MOCK_AVAILABILITY_BY_OPERATOR: Record<number, Record<string, FspAvailability>> = {
  ...
  1002: {
    'inst-101': { userGuidId: 'inst-101', availabilities: weekdayAvail(7, 16), availabilityOverrides: [] },
    'inst-102': { userGuidId: 'inst-102', availabilities: weekdayAvail(8, 18), availabilityOverrides: [] },
    'stu-101': { userGuidId: 'stu-101', availabilities: weekdayAvail(9, 17), availabilityOverrides: [] },
    'stu-102': { userGuidId: 'stu-102', availabilities: weekdayAvail(8, 15), availabilityOverrides: [] },
    'stu-103': { userGuidId: 'stu-103', availabilities: weekdayAvail(10, 18), availabilityOverrides: [] },
  },
  1003: {
    'inst-201': { userGuidId: 'inst-201', availabilities: weekdayAvail(7, 17), availabilityOverrides: [] },
    'stu-201': { userGuidId: 'stu-201', availabilities: weekdayAvail(8, 16), availabilityOverrides: [] },
    'stu-202': { userGuidId: 'stu-202', availabilities: weekdayAvail(9, 17), availabilityOverrides: [] },
  },
};
...
let _lastLoginEmail = '';
export function setLastLoginEmail(email: string) { _lastLoginEmail = email; }
export function getLastLoginEmail(): string { return _lastLoginEmail; }
